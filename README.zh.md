...
该示例连接本地 Ollama 服务；`REEF_MULTIMODAL_API_KEY` 是一个 OpenRouter key，让 harness 的 extension
能生成图片和语音、做文本 embedding、调用决策模型，key 由 Reef 持有，extension 看不到它。不需要这些时省略它；上游是
