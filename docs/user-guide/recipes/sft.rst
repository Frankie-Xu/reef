...
self-distillation comparisons: it takes the report the ``sdft`` recipe takes
(a rollout's receipt and the demonstration as ``context``), so a harness
drives both methods with the same feedback and the difference between them
is the loss alone.
...
| Needs       | GPUs                                                       |
+-------------+------------------------------------------------------------+
| Example     | the SFT arm of `SDFT on a skill stream                     |
...
The same contract as the ``sdft`` recipe: a report references one inference
record and carries the demonstration as ``metadata.context``. A ``score`` is
...
- `SDFT <sdft.rst>`__: the on-policy method this recipe is the control for,
  on the same reports.
