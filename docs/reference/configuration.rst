...
.. config::
   ...
   evolution.task_manifest | a split manifest written by ``reef.core.tasks``; the eval side names the gate's tasks as directories under ``evolution.tasks_root``, each passed to the adapter as its path; set instead of ``evolution.tasks``
