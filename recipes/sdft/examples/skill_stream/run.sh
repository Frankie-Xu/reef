#!/usr/bin/env bash
# One arm of the skill stream (Figure 3 of arXiv:2601.19897) through reef-eval:
# run.py starts the arm's Reef stack per stage and runs each stage as a
# Harbor task. Setup (once): see README. State goes to $RUN_DIR.
#
#   ./run.sh --arm sdft          the SDFT arm: Tool Use, then Science Q&A
#   ./run.sh --arm sft           the SFT control on the same stack
set -euo pipefail
cd "$(dirname "$0")"
