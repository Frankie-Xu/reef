...
The first generation starts when the processor first looks for a batch; the next after `batches-per-generation` batches were acknowledged, so the Designer writes for the policy that trains now; `generations` caps them. `GET /reef/status` shows the generation in flight, the count completed and the last error.
