...
- Retryable API errors (5xx, 429, connection and timeout errors) back off and
  retry without limit; any other error is fed back to the model as a user
  turn and the turn is regenerated.
...
class BashAgent:
    ...
    def _refresh_context(self) -> None:
        """A new week: the conversation restarts from the system prompt."""
    ...
    def check_day_advanced(self, bash_output: str) -> bool:
        ...
        if match and int(match.group(1)) > self.current_day:
            ...
            self._new_dashboard = bash_output[bash_output.index(match.group(0)) :]
            return True
        return False
    ...
    def _record_usage(self, response: Any) -> None:
        ...
        self.last_input_tokens = int((getattr(usage, "prompt_tokens", 0) or 0) if usage else 0)
        self.last_output_tokens = int((getattr(usage, "completion_tokens", 0) or 0) if usage else 0)
        self.last_cached_tokens = int((getattr(prompt_details, "cached_tokens", 0) or 0) if prompt_details else 0)
            ...
            try:
                ...
                if calls:
                    ...
                    for call in calls:
                        ...
                        extra_content = (getattr(call, "model_extra", None) or {}).get("extra_content")
                        if extra_content:
                ...
                self._pending_tool_calls = [{"id": first.id, "name": first.function.name}]
                return Action(tool=first.function.name, arguments=arguments)
            except Exception as error:  # the benchmark's loop treats every failure this way
                status = int(getattr(error, "status_code", 0) or 0)
                retryable = isinstance(error, openai.APIStatusError) and (status >= 500 or status == 429)
                ...
                if retryable:
                    wait = min(
                        RETRY_BACKOFF_CAP_S,
                        RETRY_BACKOFF_BASE_S * 2 ** min(self._consecutive_errors - 1, RETRY_BACKOFF_STEPS),
                    )
                    self.logger.warning("model call failed (%s); retrying in %ds", error, wait)
                    time.sleep(wait)
                    continue
                wait = min(FEEDBACK_WAIT_CAP_S, FEEDBACK_WAIT_STEP_S * self._consecutive_errors)
