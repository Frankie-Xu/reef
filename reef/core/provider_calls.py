...
unchanged to the deployment's multimodal provider (:mod:`reef.inference.multimodal`),
whatever serves the chat, and relays the response byte for byte; the record keeps
a summary instead of the body, so generated media and embedding vectors never
enter record storage or a proposer's prompt. The record names its route under
